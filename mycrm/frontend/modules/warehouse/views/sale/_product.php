...
<tr class="dynamic-product">
    ...
    <td class="error-popover">
        <?= $form->field($model, "[{$index}]quantity",
            ['template' => "{input}\n{error}"])->textInput(['style' => 'width:50px', 'class' => 'product-quantity']) ?>
    </td>
    <td>
        <?= $form->field($model, "[{$index}]price",
            ['template' => "{input}\n{error}"])->textInput(['style' => 'width:80px', 'class' => 'product-price']) ?>
    </td>
    <td>
        <?= $form->field($model, "[{$index}]discount",
            ['template' => "{input}\n{error}"])->textInput(['style' => 'width:80px', 'class' => 'product-discount']) ?>
    </td>
    <td class="product-cost"><?= $model->getFinalPrice() ?></td>
    <td class="product-stock-level"><?= isset($model->product) ? $model->product->quantity : '' ?></td>
